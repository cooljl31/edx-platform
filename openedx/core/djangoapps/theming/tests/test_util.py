...
def compile_sass(system, themes=None, force=False):
    """
    Process xmodule assets and compile sass files. Make sure to update assets only once even if called multiple times.

    Args:
         system (str): 'lms' or 'cms', specified the system to compile sass for.
         themes (list): list of all the themes to compile sass for.
         force (Bool): if True than force compile all sass even it has already been compiled.
    """
    theme_names = ' '.join(themes) if themes else 'no'

    # Compile sass if it has not been compiled already or of 'force' is true
    if force or not COMPILED_SASS.get(system + theme_names, False):
    ...
    COMPILED_SASS[system + theme_names] = True
